...
def test_generate_character_mapping():
    mapping = generate_dataframe()
    assert len(mapping) == 26  # Check if there are 26 letters
    assert all(len(t[1]) == 9 for t in mapping)  # Check if each letter has 8 values
    assert all(
        t[0] in string.ascii_lowercase for t in mapping
    )  # Check if all letters are lowercase
    assert len(set(t[0] for t in mapping)) == len(
        mapping
...
def test_generate_alphabet_df():
    ...
    assert len(df.columns) == 9  # Check if there are 9 columns
    assert all(
        df["carrier_letter"].apply(lambda x: x in string.ascii_lowercase)
    )  # Check if all letters are lowercase
